  ...
  try {
    ...
    if (emailQueue) {
      emailQueue.add('sendEmail', {
        ...
        body: `Hi ${name}, thank you for registering on UptoSkills LMS. Your account status is: ${userStatus}.`
      }).catch(err => console.error('Error queuing registration welcome email:', err));
    }
  ...
  try {
    ...
    if (emailQueue) {
      emailQueue.add('sendEmail', {
        ...
        body: `Click here to reset your password: http://localhost:8080/reset-password/${user.id}/${token}`
      }).catch(err => console.error('Error queuing password reset email:', err));
    }

    res.status(200).json({ 
      success: true, 
      message: 'If that email is registered, a reset link has been sent.',
      resetLink: `http://localhost:8081/reset-password/${user.id}/${token}` // FOR TESTING ONLY
    });
  } catch (error) {
    next(error);
  }
