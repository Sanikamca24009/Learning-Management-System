...
const InstructorPortal = () => {
  ...
  const fetchCourses = useCallback(async () => {
    setIsLoading(true);
    try {
      const res = await courseApi.getMyCourses();   // returns all statuses for this instructor
      const all: CourseItem[] = res.data.data;
      setCourses(all);
      if (!lessonForm.courseId && all.length > 0) {
        setLessonForm((f) => ({ ...f, courseId: all[0].id }));
      }
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">
                    {c.category} · {c.instructor?.name || "Unknown"}
                  </p>
                </div>
                ...
                <div className="hidden sm:flex items-center gap-3 text-xs text-muted-foreground shrink-0">
                  ...
                  <span className={`px-2 py-0.5 rounded-full text-[10px] font-semibold border ${
                    (c as any).status === 'approved'  ? 'border-green-500/40 text-green-400 bg-green-500/10' :
                    (c as any).status === 'rejected'  ? 'border-destructive/40 text-destructive bg-destructive/10' :
