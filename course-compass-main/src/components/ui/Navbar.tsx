...
export const Navbar = () => {
  ...
  const visibleLinks = links.filter((l) => {
    if ((user?.role === "admin" || user?.role === "instructor") && (l.label === "Dashboard")) {
      return false;
    }
    return true;
  });
    ...
    <header className="sticky top-0 z-50 backdrop-blur-xl bg-background/70 border-b border-border">
      <div className="container flex items-center justify-between h-16">
        <NavLink to="/" className="flex items-center gap-2 font-display font-bold text-lg">
          <div className="w-9 h-9 rounded-lg bg-primary flex items-center justify-center shadow-[0_0_20px_hsl(var(--primary)/0.5)]">
            <GraduationCap className="w-5 h-5 text-primary-foreground" />
          </div>
          <span>Upto<span className="text-primary">Skills</span></span>
        </NavLink>
        ...
        <nav className="hidden md:flex items-center gap-8">
          ...
          {/* Portal link — instructors and admins only */}
