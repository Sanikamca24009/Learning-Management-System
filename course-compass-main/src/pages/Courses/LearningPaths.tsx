...
const LearningPaths = () => {
  ...
  const [learningPaths, setLearningPaths] = useState<DynamicPath[]>([]);
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      try {
        ...
        // Group by category
        const pathsMap: Record<string, Course[]> = {};
        mapped.forEach(c => {
          if (!pathsMap[c.category]) pathsMap[c.category] = [];
          pathsMap[c.category].push(c);
        });

        // Generate paths
        const paths = Object.keys(pathsMap).map((category, idx) => ({
          id: category.toLowerCase().replace(/\s+/g, '-'),
          title: `${category} Path`,
          description: `Master ${category} from beginner to advanced.`,
